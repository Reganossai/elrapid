...
const TwoMinuteDrillList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(
      `http://localhost:5000/api/2mindrill/delete/${id}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    if (res.ok) {
      alert("🗑️ Deleted!");
      setDrills(drills.filter((d) => d._id !== id));
    } else {
    ...
    <div className="cal-tab">
      ...
      <table>
        ...
        <tbody>
          {drills.map((drill) => (
            <tr key={drill._id}>
              <td>{drill.individual}</td>
              <td>{drill.date}</td>
              <td>
                <button
                  onClick={() => navigate(`/2mindrill/edit/${drill._id}`)}
                >
                  Edit
                </button>
                <button
               ...
               <button onClick={() => exportDrillPDF(drill)}>Export PDF</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default TwoMinuteDrillList;
